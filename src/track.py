...
MOUNTAIN_RIDGE_FEATHER_PX = 4     # 稜線を縦方向へにじませる幅（px）。1でほぼAAのみ、大きいほど空へ溶ける
...
class Track:
    ...
    def _blit_mountain_aa(self, screen, mountain_poly, ridge_pts, color, screen_width, screen_height):
        ...
        # にじみが上下へ広がるぶん帯を余分に取る（足りないと、にじみが帯の縁で切れる）
        feather = max(1, MOUNTAIN_RIDGE_FEATHER_PX)
        y0 = max(0, int(top) - feather - 1)
        y1 = min(screen_height, int(bot) + feather + 2)
        if y1 <= y0:
            return False
        ...
        blurred = pygame.transform.smoothscale(band, (screen_width, max(1, h // feather)))
